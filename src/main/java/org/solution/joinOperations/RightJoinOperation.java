...
public class RightJoinOperation<D1, D2, R> implements JoinOperation<D1, D2, R> {
    @Override
    public Collection<R> join(Collection<D1> leftCollection, Collection<D2> rightCollection) {
        ...
        for (Integer i : set2) {
            answer.add(new JoinedDataRow<>(i, (set1.contains(i) ? list1.get(possibleKeys2.get(i)).getValue() : null) ,
                    list2.get(possibleKeys2.get(i)).getValue()));
        }
